...
import { Card, Table, Row, Col, Statistic, Tag, Spin, Button, Dropdown, DatePicker, Select, Space, Empty } from 'antd';
import {
  ArrowUpOutlined,
  ArrowDownOutlined,
  DollarOutlined,
...
export default function Report() {
  ...
  const sektorOptions = useMemo(() => {
    ...
    for (const s of sahamList) {
      if (s.sektor?.nama) set.add(s.sektor.nama);
    }
    return Array.from(set).sort().map((nama) => ({ value: nama, label: nama }));
  }, [sahamList]);

  useEffect(() => {
    api.get('/saham').then((res) => setSahamList(res.data));
  }, []);
  ...
  const filteredTransaksi = useMemo(() => {
    ...
    if (selectedSektor.length > 0) {
      const sahamInSektor = new Set(
        sahamList.filter((s: any) => selectedSektor.includes(s.sektor?.nama)).map((s: any) => s.kode)
      );
      t = t.filter((tr: any) => sahamInSektor.has(tr.saham?.kode));
    }

    return t;
  }, [allTransaksi, generated, dateRange, selectedSektor, sahamList]);

  const data = useMemo(() => {
    if (!generated) return [];
    const t = filteredTransaksi;
    ...
    for (const [kode, v] of map) {
      ...
      const hargaSaatIni = savedPrices[kode] || hargaRata;
    ...
    return items;
  }, [filteredTransaksi, generated, sahamList]);
  ...
  const columns: ColumnsType<ReportItem> = [
    ...
    {
      title: 'Dividend', key: 'yield', align: 'right',
    ...
    {
      title: 'Return', key: 'return', align: 'right',
      render: (_: unknown, r: ReportItem) => (
        <div>
          <Tag color={r.returnPct >= 0 ? 'success' : 'error'}>{r.returnPct >= 0 ? '+' : ''}{r.returnPct.toFixed(2)}%</Tag>
  ...
  const downloadExcel = () => {
    const wsData = data.map((d) => ({
      Kode: d.kode, 'Nama Saham': d.nama, Sektor: d.sektor, Lembar: d.lembar,
      'Harga Rata-rata': d.hargaRata, 'Total Modal': d.modal,
      'Market Price': d.hargaSaatIni, 'Nilai Pasar': d.nilaiPasar,
      'Total Dividen': d.totalDividen, 'Dividend Yield': Number(d.yield.toFixed(2)),
      'Return (Rp)': d.returnRp, 'Return (%)': Number(d.returnPct.toFixed(2)),
    }));
    wsData.push({
      Kode: 'TOTAL', 'Nama Saham': '', Sektor: '', Lembar: data.reduce((s, d) => s + d.lembar, 0),
      'Harga Rata-rata': 0, 'Total Modal': totalModal,
      'Market Price': 0, 'Nilai Pasar': totalPasar,
      'Total Dividen': totalDividen, 'Dividend Yield': Number(avgYield.toFixed(2)),
      'Return (Rp)': totalReturnRp, 'Return (%)': Number(totalReturnPct.toFixed(2)),
    });
  ...
  const downloadPdf = () => {
    ...
    if (dateRange?.[0] && dateRange?.[1]) {
      doc.text(`Periode: ${dateRange[0].format('DD/MM/YY')} - ${dateRange[1].format('DD/MM/YY')}`, 14, 34);
    }
  ...
  return (
    <>
      <Card style={{ marginBottom: 16 }}>
        <Space wrap size="middle">
          ...
          <Select
            ...
            onChange={setSelectedSektor}
            style={{ minWidth: 200 }}
            allowClear
            maxTagCount={2}
          />
          <Button type="primary" icon={<FilterOutlined />} onClick={generate} loading={loading}>
            Generate Report
          </Button>
        </Space>
      </Card>
      ...
      {!generated ? (
        <Card>
          <Empty description="Atur filter di atas lalu klik Generate Report" />
        </Card>
      ) : (
        <Spin spinning={loading}>
          {data.length === 0 ? (
            <Card><Empty description="Tidak ada data untuk filter yang dipilih" /></Card>
          ) : (
            <>
